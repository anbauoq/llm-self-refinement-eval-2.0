...
def load_all_hint_sentences(results_root):
    """
    Load all hint_sentence strings from hints.jsonl across all model/dataset/max_tokens runs.
    Returns:
      corpus: list[str] — one hint text per item (non-empty only)
      meta: pd.DataFrame — columns model, dataset, max_tokens, hint_text (index aligned with corpus)
    """
    rows = []
    for model in sorted(os.listdir(results_root)):
        ...
        for dataset in sorted(os.listdir(model_path)):
            ...
            for max_tokens in sorted(os.listdir(dataset_path)):
                ...
                if not os.path.isfile(hints_p):
                    continue
                for obj in iter_jsonl(hints_p):
                    hint = (obj.get("hint_sentence") or "").strip()
                    if not hint:
                        continue
                    rows.append({
                        "model": model,
    ...
    corpus = meta["hint_text"].tolist() if len(meta) else []
    return corpus, meta
...
def build_metrics_all_questions(results_root):
    ...
    Uses:
      ...
      - initial_accuracy = n_correct_init / n_questions *100
      ...
      - token means: initial/post from full_output; hint_tokens_mean from hint_sentence (same as per_question)
    """
    from transformers import AutoTokenizer
    ...
    for model in sorted(os.listdir(results_root)):
        ...
        for dataset in sorted(os.listdir(model_path)):
            ...
            for max_tokens in sorted(os.listdir(dataset_path)):
                ...
                initial_accuracy = (n_correct_init / n_questions)*100 if n_questions else 0.0
                post_hint_accuracy = ((n_correct_init + n_corrected) / n_questions)*100 if n_questions else 0.0
                delta_accuracy = (post_hint_accuracy - initial_accuracy)
                correction_rate = (n_corrected / n_incorrect_initial)*100 if n_incorrect_initial else 0.0

                # token means over ALL outputs (hint_sentence for hints, same as per_question df)
                init_texts_all = list(init_out.values())
                post_texts_all = list(post_out.values())
                hint_texts_all = [obj.get("hint_sentence", "") for obj in iter_jsonl(hints_p)]
...
def main():
    ...
    parser.add_argument("--per_question_out", type=str, default="evaluation/eval_results/per_question_tokens.csv")
    parser.add_argument("--metrics_out", type=str, default="evaluation/eval_results/metrics.csv")
